...
test.describe('Test rating-functionality', () => {
	test('Test add a rating', async ({ page }) => {
		 ...
		 */
		await page
			.getByRole('heading', { name: 'Byggryn Hele 550g boks Møllerens' })
			.click();
	...
	test('Test add several ratings', async ({ page }) => {
		...
		await page.goto('./results');
		await page
			.getByRole('heading', { name: 'Weetos Choco 375g Weetabix' })
			.click();
		await checkIfRatingIconsAreCorrect(page, 0);
		...
		await expect(
			page.getByText('Weetos Choco 375g Weetabix'),
		).toBeVisible();
