...
test.describe('Test favorite-functionality', () => {
	test('Test favorisering', async ({ page }) => {
		...
		await expect(page.getByLabel('like')).toHaveAttribute(
			'style',
			'background-color: grey; outline: none;',
		);
		...
		await expect(page.getByLabel('like')).toHaveAttribute(
			'style',
			'background-color: rgb(40, 112, 148); outline: none;',
		);
		...
		expect(await products[0].innerText()).toContain(
			'Byggryn Hele 550g boks Møllerens',
		);
		await page.getByText('Byggryn Hele 550g boks Møllerens').click();
