...
export async function getProductsWithName(page: PageType, name: string): Promise<PageType[]> {
...
export async function checkIfRatingIconsAreCorrect(page: PageType, rating: number): Promise<void> {
    const index = rating - 1;
    if (index === -1) {
    for (let i = 0; i < 5; i++) {
        await expect(page.locator('.MuiRating-icon').nth(i)).toHaveClass(/MuiRating-iconEmpty/);
    }
    } else if (index >= 0 && index <= 4) {
    for (let i = 0; i < 5; i++) {
        if (i === index) {
            await expect(page.locator('.MuiRating-icon').nth(i)).toHaveClass(/MuiRating-iconFilled/);
    } else {
        await expect(page.locator('.MuiRating-icon').nth(i)).toHaveClass(/MuiRating-iconEmpty/);
    }
    }
