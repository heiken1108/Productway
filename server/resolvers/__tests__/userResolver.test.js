	...
	describe('UserQuery', () => {
		describe('getFavoritesByUserID', () => {
			it('Should get all favorites by userID', async () => {
				const result = await userQuery.getFavoritesByUserID(null, {
					userID: 'user456',
				});
		...
		describe('getRatingsByUserID', () => {
			it('Should get all ratings by userID', async () => {
				const result = await userQuery.getRatingsByUserID(null, {
					userID: 'user456',
				});
	...
	describe('UserMutation', () => {
		...
		describe('addFavorite', () => {
			it('Should add favorite to user', async () => {
				...
				const result = await userMutation.addFavorite(null, {
					userID: 'user456',
					productID: 2,
				});
				...
				expect(updatedUser.favorites[0].toString()).toBe(
					result.favorites[0].toString(),
				);
			});
		});
	});
