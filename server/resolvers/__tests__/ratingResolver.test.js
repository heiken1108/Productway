    ...
    describe('RatingQuery', () => {
        describe('getRatingByProductIDandUserID', () => {
            it('Should return rating based on ProductID and UserID', async () => {
                ...
                const result = await ratingQuery.getRatingByProductIDandUserID(null, {productID: 1, userID: 'user123'})

                const isRating = mockRatings.some((mockRatings) => mockRatings.rating === result.rating)
        ...
        describe('getAverageProductRating', () => {
            it('Should return average rating based on ProductID', async () => {
                await createRatings(mockRatings);
                const result = await ratingQuery.getAverageProductRating(null, {productID: 1})
    ...
    describe('RatingMutation', async () => {
        describe('addRating', () => {
            it('Should add a new rating', async () => {
                ...
                const addedRating = await RatingModel.findOne({ productID: newRating.productID, userID: newRating.userID });
            ...
            it('Should update an existing rating', async () => {
                ...
                const result2 = await ratingMutation.addRating(null, existingRating);

                const updatedRatingInDB = await RatingModel.findOne({ productID: existingRating.productID, userID: existingRating.userID });
        ...
        describe('removeRating', () => {
            it('Should remove a rating based on ProductID and UserID', async () => {
                ...
                await UserModel.updateOne({ userID: 'user456' }, { $push: { ratings: createdRating._id } });


                const result = await ratingQuery.getRatingByProductIDandUserID(null, {productID: 2, userID: 'user456'})
                const result2 = await ratingMutation.removeRating(null, {productID: 2, userID: 'user456'});
                const result3 = await ratingQuery.getRatingByProductIDandUserID(null, {productID: 2, userID: 'user456'})
