...
const productResolver = {
	Query: {
		 ...
		 */
		getProductsByFiltersWithLimit: async (
			parent,
			{
				searchTerm,
				categories,
			...
			let products;
			if (sortOrder === 1) {
				products = await ProductModel.find(filters)
					.sort({ currentPrice: 1 })
					.limit(limit)
					.skip((page - 1) * limit); // Stigende rekkefølge
			} else if (sortOrder === -1) {
				products = await ProductModel.find(filters)
					.sort({ currentPrice: -1 })
					.limit(limit)
					.skip((page - 1) * limit); // Synkende rekkefølge
			} else {
				products = await ProductModel.find(filters)
					.limit(limit)
					.skip((page - 1) * limit); // Ingen spesifisert rekkefølge, returner som det er
			}
			return products;
		},
		/**
		 ...
		 */
		getCountProductsByFilters: async (
			parent,
			{ searchTerm, categories, minPrice, maxPrice },
		) => {
			const filters = {};
			if (searchTerm) {
				const regex = new RegExp(searchTerm, 'i');
